...
function BlogListItem(item) {
  ...
  return (
    <div className="border font-light 2xl:space-y-4 space-y-2 hover:border-slate-400 bg-gray-400/75 dark:bg-slate-800 dark:border-white/5 border-black-400 rounded p-1 lg:p-4">
      <div>
        <Link href={`/blog/${slug}`}>
      ...
      <div className="italic text-base 2xl:text-lg xl:text-md lg:text-md">{item.summary}</div>
    </div>
  );
}
